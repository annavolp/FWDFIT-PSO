...
; returns a structure whose main fields are 'circle', 'ellipse', 'loop', replicated according to the number of times they are 
; repeated in the configuration. Each shape contains 3 fields: parameters to be optimised  characteristic for each shape (param_opt), 
; lower and upper bound for each variable.
; 
; For example: if configuration=['circle', 'ellipse', 'circle'], srcin has two main fields: srcin.circle and srcin.ellipse.
